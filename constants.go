...
var gIntFacts = []sCustomPrompt{
	...
	},
	{
		...
		Prompt: [][]openai.ChatCompletionMessage{
			...
			{
				{Role: openai.ChatMessageRoleUser, Content: "Придумай новость об актуальных событиях в мире и прокоментируй её. Начни с фразы типа 'Новости подкатили!' и отступи строку."},
				...
				{Role: openai.ChatMessageRoleUser, Content: "Придумай новость об актуальных событиях в мировой экономике. Начни с фразы типа 'Считаем денежки!' и отступи строку."},
		...
		Prompt: [][]openai.ChatCompletionMessage{
			...
			},
			{
				{Role: openai.ChatMessageRoleUser, Content: "Исходя из контекста педыдущего сообщения определи - что требуется и поясни почему:\n" +
					...
					"- пользователь просит новости, то ответь 'Сайт'\n" +
